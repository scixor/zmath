...
noinline fn mat4MulBenchmark(allocator: std.mem.Allocator, io: std.Io, comptime count: comptime_int) !void {
...

    {
        ...
        const elapsed_s = @as(f64, @floatFromInt(timer.read())) / time.ns_per_s;
...
noinline fn waveBenchmark(io: std.Io, comptime count: comptime_int) !void {
